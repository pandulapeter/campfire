...
```
Offer to connect again instead of only Disconnect when Dropbox refuses a sync run.
Keep unsaved editor text through a rotation after the song's file is gone.
Add the iOS privacy manifest that App Store Connect requires.
Read ChordPro directives written with a space instead of a colon.
Let a setlist be reordered from its row's menu and with a screen reader.
Stop the library size in Settings listing the whole library on every change.
```

## 4. Subagent prompt (orchestrator fills in the three placeholders)

```
You are working on lane {LANE} of the Campfire fourth pre-release review, in the git worktree at {WORKTREE_PATH}
...
its port); if the Kotlin incremental cache errors, rerun with -Pkotlin.incremental=false. Never a
...
As each subagent reports, in the order **B, E, A, C, D** (a lane that finishes early waits for its turn; A reaches
the view model through 45, C carries the view model, and D edits strings and screens that A and C have been in, so
those go last):
...
1. In the lane's worktree: `git rebase master`. Resolve conflicts by keeping both sides' intent (the touch points in
   ...
   A lane rebased onto lane A's 45 may no longer compile where it builds a `Song` or a `Setlist`: add the `size`
   argument as 45 did for the test helpers, in the commit that constructs it.

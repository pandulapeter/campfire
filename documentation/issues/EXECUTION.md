...
1. `git status` is clean apart from `documentation/issues/`, and the current branch is `master`. If not, stop and
   tell the user. In particular, the end-to-end test work that was uncommitted while this review ran (`SyncEngine.kt`,
   `SyncProvider.kt`, `DropboxSyncProvider.kt`, `CampfireDesktopApplication.kt`, `TestDriver.kt`, `sync.md`,
   `documentation/testing/`, and the others `README.md` lists) must be committed or abandoned first — never stash
   or discard it yourself.
2. The user has seen `README.md`'s "Defaults chosen while writing" — plan 17's recommendation above all. If the user
   has not said to go ahead with them, stop and ask.
3. The unit tests pass on `master` (the command is in `README.md`). If not, stop and tell the user.
4. Note the starting commit: `git rev-parse HEAD` → `START`. Every later check compares against it.
5. `local.properties` is gitignored and will not be in the worktrees; that is fine (the build falls back to the debug
   keystore and an empty Dropbox key). Only lanes B and D benefit from a real Dropbox key for manual checks; copy the
   file into their worktrees if it exists: `cp local.properties <worktree>/`.
6. The plan files are untracked. Commit them first, on `master`, so every lane branches from a tree that has them:
   `git add documentation/issues && git commit -m "Add the sixth pre-release review plans."`
...
Repeat for each issue number in the lane's list, **in the order given** (several lanes are deliberately not in numeric order; see `README.md`), in the
...
5. Verify:
   ...
   - if it touches the desktop entry point or the packaged runtime (45):
   ...
   - the manual check the plan describes, where a desktop run can do it (`./gradlew :app:desktop:run`); skip manual
     ...
     pipeline change (49): read the YAML and reason, and say in the report that it is unproven until a release.
...
The plans carry no suggested message; write one from the plan's title, stated as what the change does. Examples
from the previous round, for the voice:
...
```
You are working on lane {LANE} of the Campfire sixth pre-release review, in the git worktree at {WORKTREE_PATH}
...
from `README.md`'s table, in its order — lane B is `10, 11, 09, 16, 17, 14, 12, 13, 15, 18, 19, 20`).

## 5. Merging a finished lane (orchestrator)

As each subagent reports, in the order **B, C, A, E, D** (a lane that finishes early waits for its turn: B is the
storage and sync base C's 27 and 28 build on, C is the domain, A is `:chordpro` and one view-model function, E is docs,
CI, the web worker and the desktop entry point, and D edits the view model, `CampfireApp.kt` and the editor more than
anyone and goes last):
...
1. In the lane's worktree: `git rebase master`. Resolve conflicts by keeping both sides' intent (`README.md`'s
   ...
   Specific ones to expect: C rebased onto B finds `SongLocalSourceImpl.renameSong` and `SyncRepositoryImpl` changed
   by B's 14, 18 and 19 (C's 27 and 28 must keep B's behaviour); C's test fakes may need members B added to the local
   source (`loadSongFileSizes` is C's own); A and D both edit `CampfireViewModel.kt` (A's 02 in `renderSong` only);
   D's `strings.xml` changes sit next to B's 19 and C's 23.
...
3. Report to the user: the number of commits, the issues skipped with the reason from each lane's report, the manual
   ...
   this lands, the riskiest plans in it — 21 (import Replace), 44 (the kept draft) and 17 (case-only
   renames) above all — have still never been run against a real account or a real device. Do not push.

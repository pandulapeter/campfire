...
fixed issue deleted in the commit that fixes it. When all seven lanes are merged, `documentation/issues/` is removed
in a final commit. Nothing is pushed.
...
4. `local.properties` is gitignored and will not be in the worktrees; that is fine (the build falls back to the
   debug keystore and an empty Dropbox key). Only lanes A and B benefit from a real Dropbox key for manual checks;
   copy the file into their worktrees if it exists: `cp local.properties <worktree>/`.
...
Then spawn **seven subagents at once** with the `Agent` tool (`subagent_type: general-purpose`, same model as
...
E–G when two of them have finished.
...
Placeholders: `{LANE}` (A–G), `{WORKTREE_PATH}` (absolute path of the worktree), `{ISSUE_LIST}` (the lane's numbers
from `README.md`'s table, in its order).

## 5. Merging a finished lane (orchestrator)

As each subagent reports, and **lane G last**:

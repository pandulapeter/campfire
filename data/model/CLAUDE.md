...
- `domain/` — `Song` (the metadata of one `.cho` file, without its text, tags included, plus `canUpdateFileName` — whether the file is named something other than what its own metadata would name it, worked out once per scan because every row of the song list asks it), `SongContent` (that text), `Setlist` (with per-entry transposition, so it travels with the setlist, `isArchived`, so a setlist put away on one device is put away on every other one it syncs to, and the optional `description`, which travels the same way and is what the setlists screen's search reads besides the title), `Tag` (one label with the number of songs carrying it, which is all the filter controls need), `SongLanguage` (the same for a language), `UserPreferences` (with `SortingMode`, `SetlistSortingMode`, `UiMode`, `ThemeColor`, `Language`, `Accidentals` and `TagMatchMode` enums whose `id` values are persisted, the display-only `ChordSpelling`, `isPerformanceModeEnabled` — read only mode for the whole app, see `:presentation` — plus the library-wide transpositions and `shouldShowArchivedSetlists` — but not which tags and languages the song list is filtered to, which is never stored, see `SongFilter` in `:domain:api`), `ImportedFile` / `ExportedFile` / `ImportResult` for the import and export paths, and `ImportPlan` (what an import would do to each name, worked out before anything is written) with the `ImportConflictResolution` the user answers it with.
...
- `domain/LibraryText.kt` — `decodeLibraryText`, how the bytes of a song or setlist become text: strict UTF-8, Windows-1252 for a file that is not valid UTF-8, and no byte order mark. The storage layer reads every library file through it and the import every incoming file, so a file reads the same whichever way it arrived. It is tested from `:data:source:local:implementation`, since this module has no tests of its own.

...
- `domain/` — `Song` (the metadata of one `.cho` file, without its text), `SongContent` (that text), `Setlist` (with per-entry transposition, so it travels with the setlist), `UserPreferences` (with `SortingMode`, `UiMode`, `Language` and `Accidentals` enums whose `id` values are persisted, plus the library-wide transpositions), `ImportedFile` / `ExportedFile` / `ImportResult` for the import and export paths.

...
- `domain/` — `Song` (the metadata of one `.cho` file, without its text, tags included), `SongContent` (that text), `Setlist` (with per-entry transposition, so it travels with the setlist), `Tag` (one label with the number of songs carrying it, which is all the filter controls need), `UserPreferences` (with `SortingMode`, `UiMode`, `Language`, `Accidentals` and `TagMatchMode` enums whose `id` values are persisted, the display-only `ChordSpelling`, plus the library-wide transpositions and the selected tags), `ImportedFile` / `ExportedFile` / `ImportResult` for the import and export paths.

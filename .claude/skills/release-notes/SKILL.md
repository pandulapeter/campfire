...
7. **Write the draft** to `.release-notes/<version>.md` (create the directory; `/.release-notes/` is
   ...
   Then write `.release-notes/<version>-play.md`: two lines, built for the `release_notes` /
   `release_notes_hu` inputs of `android-publish.yml` (see below). Tell the user both paths and that
   they are throwaways to delete after pasting.
...
`android-publish.yml`'s `release_notes` / `release_notes_hu` workflow-dispatch inputs are single-line
text fields: each is pasted in as one line with literal `\n` sequences for line breaks, and the workflow
expands them with `printf '%b'` before Play sees them. So `<version>-play.md` is **two lines**, each
paste-ready for one input, not a bulleted markdown file:

- **Line 1 — English.** The release's bullets, trimmed to the ones a Play Store visitor would care
  about, in the same voice as the GitHub notes, joined into one line as `- bullet one\n- bullet two\n...`
  (a literal backslash-n between bullets, no real line breaks in the file).
- **Line 2 — Hungarian.** A translation of the same bullets into Hungarian, in the same one-line
  `- bullet\n- bullet` shape. Translate it yourself; don't leave it for the user unless asked to.
- **500 characters at most per line, counting each `\n` as the one newline character it expands to —
  not as two.** Check both lines with a real interpreter (e.g. Python's `len()` on the string with `\n`
  replaced by an actual newline), not by eyeballing it, and report both counts back. Hungarian tends to
  run longer than English for the same content, so its bullets usually need to be trimmed tighter to fit
  — shorten wording before dropping a bullet.
- **No markdown**: no links, no bold, no backticks. Plain `-` bullets and plain text only, on both lines.
